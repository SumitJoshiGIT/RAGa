...
class PineconeAsyncClientParent:
    def __init__(self,embedder,index_name="index", text_index_name=None):
...
class PineconeAsyncClient(PineconeAsyncClientParent):
    async def insert(self, input: bytes, metadata: dict = None):
        embedding = await self.embedder.embed(input)
        vector = {
            "id": str(uuid.uuid4()),
            "values": embedding,
